...
uint32_t zero_return_speed_value=20;//回零速度（单位0.001mm/s）
uint32_t running_speed_value=10;//运行速度
...
uint8_t Detained_pulse_flag=0;//滞留脉冲清楚标志位
...
uint8_t Iwdg_Number=0;//看门狗计数
uint8_t flag_Motor_deceleration_stops=0;//电机减速停止标志位
uint8_t Sports_status_flag=0;//运动状态标志位
uint8_t Sports_direction_flag=0;//运动方向标志位
/* USER CODE END PTD */
...
osThreadId_t Detained_pulseTaskHandle;
const osThreadAttr_t Detained_pulseTask_attributes = {
  .name = "Detained_pulseTask",
  .stack_size = 128 * 4,
  .priority = (osPriority_t) osPriorityLow2,
};
...
void Detained_pulse(void *argument);//滞留脉冲清除
...
void MX_FREERTOS_Init(void) {
  ...
  /* USER CODE BEGIN RTOS_THREADS */
	...
	Detained_pulseTaskHandle = osThreadNew(Detained_pulse, NULL, &Detained_pulseTask_attributes);
					...
					if(Drive_control_array[8]==0x20)//判断起始地址
					{
						switch(Drive_control_array[9])//判断起始地址
						{
							...
							case 0x01://原点回归/停止
								...
								send_buff[5]=0X05;//数据长度
								send_buff[8]=0X01;//接下来数据的字节长度
								if(Sports_status_flag==0&&REACH)//回零
								{
									send_buff[9]=0X00;
								}
								else//回零中
							...
							case 0x06://伺服复位
								...
								send_buff[5]=0X05;//数据长度
								send_buff[8]=0X01;//接下来数据的字节长度
								if(Servo_reset_flag==0)//复位完成
								{
									send_buff[9]=0X01;
								}
								else if(Servo_reset_flag==1)//复位未完成
								{
									send_buff[9]=0X00;
								}													
								send(1,send_buff,10);//向客户端发送	
				...
				case 0x03://读保持寄存器(MBAP 功能码 起始地址H 起始地址L 寄存器数量H 寄存器数量L)
					...
					else if(Drive_control_array[8]==0x00)//判断起始地址
					{
						...
						else if(Data_Bytes==0x04)
						{
							...
							{
								send_buff[8]=Data_Bytes;//接下来数据的字节长度
								send_buff[11]=((zero_return_creep_speed_value*1000)>>24)&0xFF;//寄存器1H-数据
								send_buff[12]=((zero_return_creep_speed_value*1000)>>16)&0xFF;//寄存器1L-数据
								send_buff[9]=((zero_return_creep_speed_value*1000)>>8)&0xFF;//寄存器2H-数据
								send_buff[10]=(zero_return_creep_speed_value*1000)&0xFF;//寄存器2L-数据
								send(1,send_buff,13);//向客户端发送								
							}
							else if(Drive_control_array[9]==0x78)//判断起始地址——运行速度
							{		
								send_buff[8]=Data_Bytes;//接下来数据的字节长度
								send_buff[11]=((running_speed_value*1000)>>24)&0xFF;//寄存器1H-数据
								send_buff[12]=((running_speed_value*1000)>>16)&0xFF;//寄存器1L-数据
								send_buff[9]=((running_speed_value*1000)>>8)&0xFF;//寄存器2H-数据
								send_buff[10]=(running_speed_value*1000)&0xFF;//寄存器2L-数据
								send(1,send_buff,13);//向客户端发送															
							}
					...
					if(Drive_control_array[8]==0x20)//判断起始地址
					{
						switch(Drive_control_array[9])//判断起始地址
						{
							case 0x01://原点回归/停止
								...
								{
									if(AWAY&&Sports_status_flag==0)//不在原点且停止运动
									{
										send_buff[10]=0XFF;
										send_buff[11]=0X00;
...
//										Sports_status_flag=1;
										Target_Position=0;//绝对定位位置设置为0
										if(HAL_GPIO_ReadPin(IN0_GPIO_Port,IN0_Pin)==GPIO_PIN_RESET)//在尾部-正向移动
										{
											Motor_drive_send[0]=GPIO_PIN_RESET;
											Motor_drive_send[1]=((One_Circle_Distance*1000)/(zero_return_speed_value*Subtraction_2))>>8&0xff;
											Motor_drive_send[2]=(One_Circle_Distance*1000)/(zero_return_speed_value*Subtraction_2)&0xff;
...
//									Sports_status_flag=1;
									Motor_drive_send[0]=GPIO_PIN_RESET;
									Motor_drive_send[1]=((One_Circle_Distance*1000)/(running_speed_value*Subtraction_2))>>8&0xff;//自动重装载值n=(One_Circle_Distance*1000000*1000)/(running_speed_value*Subtraction)
									Motor_drive_send[2]=(One_Circle_Distance*1000)/(running_speed_value*Subtraction_2)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,Motor_drive_send,1,0);
								}
								else//停止
								{
...
//									Sports_status_flag=1;
									Motor_drive_send[0]=GPIO_PIN_SET;
									Motor_drive_send[1]=((One_Circle_Distance*1000)/(running_speed_value*Subtraction_2))>>8&0xff;
									Motor_drive_send[2]=(One_Circle_Distance*1000)/(running_speed_value*Subtraction_2)&0xff;
									osMessageQueuePut(Motor_driveQueueHandle,Motor_drive_send,1,0);								
								}
								else//停止
...
//										Sports_status_flag=1;
							...
							case 0x05://绝对运动/停止
								if(Drive_control_array[10]==0xFF)//开始
								{
									...
									{
//										Sports_status_flag=1;
										if(Position_upload*10<Target_Position||Position<0)//当前位置小于目标位置或当前位置小于0-正向移动
										{
											Motor_drive_send[0]=GPIO_PIN_RESET;
											Motor_drive_send[1]=((One_Circle_Distance*1000)/(running_speed_value*Subtraction_2))>>8&0xff;
											Motor_drive_send[2]=(One_Circle_Distance*1000)/(running_speed_value*Subtraction_2)&0xff;
										...
										else if(Position_upload*10>Target_Position&&AWAY)//当前位置大于目标位置且不在原点-反向移动
										{
											Motor_drive_send[0]=GPIO_PIN_SET;
											Motor_drive_send[1]=((One_Circle_Distance*1000)/(running_speed_value*Subtraction_2))>>8&0xff;
											Motor_drive_send[2]=(One_Circle_Distance*1000)/(running_speed_value*Subtraction_2)&0xff;
							...
							case 0x09://雷达上电/断电
								if(Drive_control_array[10]==0xFF)
								{
									send_buff[10]=0XFF;
									send_buff[11]=0X00;
									HAL_GPIO_WritePin(TIM8_CH3_GPIO_Port,TIM8_CH3_Pin,GPIO_PIN_RESET);//PC8-雷达上电（低电平上电，高电平断电）
								}
								else
								{
									send_buff[10]=0X00;
									send_buff[11]=0X00;
									HAL_GPIO_WritePin(TIM8_CH3_GPIO_Port,TIM8_CH3_Pin,GPIO_PIN_SET);//PC8-雷达上电（低电平上电，高电平断电）
								}
								send(1,send_buff,12);//向客户端发送							
								break;
...
//				{
						...
						if(Drive_control_array[8]==0x00)//判断起始地址
						{
							if(Drive_control_array[9]==0x64)//判断起始地址——回零速度
							{
								zero_return_speed_value=(Drive_control_array[13]<<8|Drive_control_array[14]|Drive_control_array[15]<<24|Drive_control_array[16]<<16)/1000;//大端传输数据处理——回零速度0.001mm/s
								send(1,send_buff,12);//向客户端发送																
							}
							else if(Drive_control_array[9]==0x68)//判断起始地址——回零蠕变速度
							{
								zero_return_creep_speed_value=(Drive_control_array[13]<<8|Drive_control_array[14]|Drive_control_array[15]<<24|Drive_control_array[16]<<16)/1000;//大端传输数据处理——回零蠕变速度0.001mm/s
								send(1,send_buff,12);//向客户端发送									
							}
							else if(Drive_control_array[9]==0x78)//判断起始地址——运行速度
							{
//								running_speed_value=(Drive_control_array[13]<<24|Drive_control_array[14]<<16|Drive_control_array[15]<<8|Drive_control_array[16]);//大端传输数据处理——运行速度0.001mm/s
								running_speed_value=(Drive_control_array[13]<<8|Drive_control_array[14]|Drive_control_array[15]<<24|Drive_control_array[16]<<16)/1000;//大端传输数据处理——运行速度0.001mm/s				
								send(1,send_buff,12);//向客户端发送		
							}							
							else if(Drive_control_array[9]==0x7A)//判断起始地址——相对定位距离
							{
...
void Motor_drive(void *argument)//电动驱动
{
	...
	for(;;)
	{  
		Sports_direction_flag=HAL_GPIO_ReadPin(TIM8_CH2_GPIO_Port,TIM8_CH2_Pin);
		...
		{	
			...
			__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number/2)-1);//修改占空比
...
//					__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number_i/2)-1);//修改占空比
//					HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
//					if(2000>number_i&&number_i>=200)
//					{
		...
		if(Encoder_Processing_flag==1)
		{
			...
			else if(AWAY)
			{
				...
				if(Zero_regression_flag==1&&Absolute_position_flag==1)//之前回过零且接收到绝对位置
				{
					if(COROTATION)//正向移动
					{
						if(flag_Motor_deceleration_stops==0&&Position_upload*10>=Target_Position)
						{
							flag_Motor_deceleration_stops=1;
						}		
					}
			...
			if(Hard_Trigger_flag==1)
			{
				...
				{
					Position_lastpulsec=Position_lastpulsec+Hard_Trigger_Interval*10;//.。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。。
					Hard_Trigger_flag=0;
					Trigger_Interval=0;					
				}			
			}
...
void Servo_reset(void *argument)//伺服复位
{
  for(;;)
  {
		...
		{
			HAL_TIM_PWM_Stop_IT(&htim8,TIM_CHANNEL_1);//关闭PWM输出
			DO_CEASE;//PA7-电机使能正输入(GPIO_PIN_RESET:开始，GPIO_PIN_SET:停止）
			Sports_status_flag=0;
			DO_Servo_Reset;	
			osDelay(500);
			DO_Servo_Reset_OFF;
...
void Iwdg_function(void *argument)//看门狗函数
{
  ...
  {
		osDelay(1);	
		if(Iwdg_Number<40)//当等于40时，则说明20min没有上位机控制信号，则单片机重启
		{	
			Iwdg_Number++;
...
void Detained_pulse(void *argument)//滞留脉冲清除
{
  ...
  {
		osDelay(1);	
		if(Detained_pulse_flag==1)
		{
			HAL_GPIO_WritePin(TIM8_CH4_GPIO_Port,TIM8_CH4_Pin,GPIO_PIN_RESET);//PC9-清除滞留脉冲（低电平清除）
			osDelay(20);
			HAL_GPIO_WritePin(TIM8_CH4_GPIO_Port,TIM8_CH4_Pin,GPIO_PIN_SET);//PC9-清除滞留脉冲
			Detained_pulse_flag=0;
		}	
	}
}
...
void Motor_deceleration_stops(void *argument)//电机减速停止
{
  ...
  {
		osDelay(1);	
		if(flag_Motor_deceleration_stops==1)//立刻停止
		{	
			HAL_TIM_PWM_Stop_IT(&htim8,TIM_CHANNEL_1);//关闭PWM输出
			Sports_status_flag=0;
